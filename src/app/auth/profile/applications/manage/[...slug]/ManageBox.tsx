...
import { AppDataType, AppRegisterFormType } from "@/types/types";
...
export default function ManageBox({ appData }: { appData: AppDataType }) {
    ...
    const [redirectUrlList, setRedirectUrl] = useState(appData.callbackUrl);
    const redirectUrlRef = useRef<HTMLInputElement | null>(null);
    const authSecretRef = useRef<HTMLParagraphElement| null>(null);

    const { control, register, handleSubmit, clearErrors, reset, setError, formState: { errors } } = useForm<AppRegisterFormType>({
        ...
        defaultValues: {
            ...
            inactiveMessage: appData.inactiveMessage,
            privacyPolicy: appData.privacyPolicy,
            website: appData.website,
            status: appData.status,
            version: appData.version,
            redirectUrl: appData.callbackUrl[0]
        }
    });
    ...
    const addRedirectUrl = () => {
        if (redirectUrlRef.current) {
            const newValue = redirectUrlRef.current.value;
            if (newValue) {
                if (redirectUrlList.includes(newValue)) return redirectUrlRef.current.ariaInvalid = "true";
                const urlSchema = yup.string().url('Invalid URL').required('URL is required');
                try {
                    redirectUrlRef.current.ariaInvalid = "false";
                    urlSchema.validateSync(newValue);
                    setRedirectUrl(prev => [...prev, newValue]);
                    redirectUrlRef.current.value = "";
                } catch (error) {
                    redirectUrlRef.current.ariaInvalid = "true";
                }
            } else redirectUrlRef.current.ariaInvalid = "true";
        }
    };

    const onSubmit: SubmitHandler<AppRegisterFormType> = async (data) => {
        setLoading(true);
        const response = await updateApp(appData.id, data, redirectUrlList);
        setEditMode(false);
        setLoading(false);
    }

    const handleReset = () => {
        setRedirectUrl(appData.callbackUrl);
        reset();
    }
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
                ...
                <section className="space-y-5">
                    <div className="space-y-2">
                        ...
                        <div className="flex items-center space-x-2">
                            ...
                            <button type="button" onClick={() => copyToClipboard(appData.id)} className="dark:text-gray-400 dark:hover:text-gray-600 transition-all">
                                <FiClipboard size={15} />
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="flex items-center space-x-2 w-[350px] md:w-[480px] lg:w-full">
                            ...
                            <button type="button" onClick={() => copyToClipboard(appData.appSecret)} className="dark:text-gray-400 dark:hover:text-gray-600 transition-all">
                                <FiClipboard size={15} />
                            </button>
                ...
                <section className="space-y-5">
                    ...
                    <div>
                        ...
                        <div className="py-3 space-y-2">
                            {redirectUrlList.map((url, index) => (
                                <div key={index} className="bg-gray-200 dark:bg-gray-800 rounded py-1 px-3 flex items-center justify-between space-x-3 w-[350px] md:w-[480px] lg:w-full">
                                    <p className="truncate">{url}</p>
                                    {isEditMode && (
                                        <button type="button" onClick={() => setRedirectUrl(prev => prev.filter(item => item !== url))} className="hover:text-gray-500 transition-all">
                                            <RxCross2 size={20} />
                                        </button>
                                    )}
                                </div>
                    ...
                    <div className="space-y-2">
                        <label className="font-medium text-emerald-500 block">Website URL</label>
            ...
            <div className="flex items-center justify-end mt-8 space-x-3">
                <button type="button" onClick={handleReset} className="px-3 py-1 lg:py-1.5 text-white dark:text-gray-200 bg-blue-600 hover:bg-blue-700 transition-all rounded flex items-center justify-center space-x-2">
                    <BiReset size={18} />
                    <p>Reset</p>
                </button>
