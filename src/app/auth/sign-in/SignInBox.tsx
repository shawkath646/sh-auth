...
import userSignInTwoStep from "@/lib/database/databaseActions/userSignInTwoStep";
import { useForm, SubmitHandler } from "react-hook-form";
import { yupResolver } from "@hookform/resolvers/yup";
import signInFormSchema from "@/lib/FormSchema/signInFormSchema";
import { ApplicationBasicDataContext } from "@/app/auth/layout";
import Countdown from "@/components/universel/Countdown";
import { UserCredintialType, StatusType, TwoStepType, ApplicationBasicDataType } from "@/types/types";
...
export default function SignInBox() {
    ...
    const onSubmit: SubmitHandler<UserCredintialExtendedType> = async (data) => {
        ...
        if (isTwoStep.isEnabled) {
            if (!data.twoStepCode || data.twoStepCode.toString().length !== 6) {
                setLoginStatus({
                    status: "error",
            ...
            response = await userSignInTwoStep({ username: data.username, password: data.password }, data.twoStepCode) as StatusType;
        } else {
            response = await userSignIn({ username: data.username, password: data.password }) as StatusType;
            if (response.twoStep) setTwoStep(response.twoStep as TwoStepType);
        }
    ...
    const handleResend = async () => {
        const response = await userSignIn({ username: getValues("username"), password: getValues("password") }) as StatusType;
        if (response.status === "two-step") setTwoStep(response.twoStep as TwoStepType);
    };
            ...
            {!isTwoStep.isEnabled && (
                <>
                    ...
                    <div className="space-y-2">
                        <button type="button" onClick={() => signIn('google')} className="w-[350px] mx-auto border border-gray-900 dark:border-gray-400 hover:border-gray-500 dark:hover:bg-gray-800 dark:hover:border-gray-800 hover:bg-gray-100 transition-all rounded-md py-1 flex items-center justify-center space-x-2">
                            <Image src={iconGoogle.src} alt="google icon" height={16} width={16} />
                            <p>Continue with Google</p>
                        </button>

                        <button type="button" onClick={() => signIn('facebook')} className="w-[350px] mx-auto border border-gray-900 dark:border-gray-400 hover:border-gray-500 dark:hover:bg-gray-800 dark:hover:border-gray-800 hover:bg-gray-100 transition-all rounded-md py-1 flex items-center justify-center space-x-2">
                            <Image src={iconFacebook.src} alt="facebook icon" height={16} width={16} />
                            <p>Continue with Facebook</p>
                        </button>

                        <button type="button" onClick={() => signIn('github')} className="w-[350px] mx-auto border border-gray-900 dark:border-gray-400 hover:border-gray-500 dark:hover:bg-gray-800 dark:hover:border-gray-800 hover:bg-gray-100 transition-all rounded-md py-1 flex items-center justify-center space-x-2">
